...
models = []  # Create a list to store all models
for bit_pos in range(8):
    print(f"Training model for bit position {bit_pos}...")
    y_train_bit = y_train_original[:, bit_pos]

    start_time = time.time()
    bit_model = svm.SVC(kernel='poly', degree=2)
    bit_model.fit(X_train, y_train_bit)
    models.append(bit_model)

    training_time = time.time() - start_time
    print(f"  Training completed in {training_time:.2f} seconds")

def predict_all_bits(models, X):
    predictions = np.zeros((X.shape[0], 8), dtype=int)

    for bit_pos in range(8):
        print(f"Making predictions for bit position {bit_pos}...")
        start_time = time.time()
        predictions[:, bit_pos] = models[bit_pos].predict(X)
...
y_pred = predict_all_bits(models, X_test)
