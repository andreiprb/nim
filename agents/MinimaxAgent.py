...
class MinimaxAgent(BaseAgent):
    ...
    """
    def __init__(self, misere, max_depth, canonical=False, P_pruning=False,
                 aggressive=False):
        """
    ...
    def _minimax(self, state, player, alpha, beta, depth):
        ...
        if self.max_depth is not None and depth >= self.max_depth:
            heuristic_score = HelperLogic.heuristic_evaluation(
                state, self.misere)
        ...
        if player == 0:
            ...
            for action in actions:
                ...
                if self.P_pruning and NimLogic.is_p_position(
                        new_state, self.misere):

                    """ MAP ACTION TO ORIGINAL STATE """
                    original_action = action if not self.canonical else (
                        HelperLogic.map_action_to_original(
                            action, index_mapping))

                    return (self.default - depth), original_action

                new_value, _ = self._minimax(
                    new_state, 1, alpha, beta, depth + 1)
                if new_value > value:
                    value = new_value

                    """ MAP ACTION TO ORIGINAL STATE """
                    best_action = action if not self.canonical else (
                        HelperLogic.map_action_to_original(
                            action, index_mapping))
            ...
            for action in actions:
                ...
                new_value, _ = self._minimax(
                    new_state, 0, alpha, beta, depth + 1)
                if new_value < value:
                    value = new_value
